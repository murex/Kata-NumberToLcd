# Getting Started with Number to LCD kata in Python
...
> ***Reminder***: the commands below should be run from [Kata-NumberToLcd/python](.) directory
...
`File` > `Open` > `Kata-NumberToLcd` > `python`
...
> ***Reminder***: the command below should be run from [Kata-NumberToLcd/python](.) directory
...
Choose `Open Folder`, navigate to `Kata-NumberToLcd` / `python` then click `Select Folder`.
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-NumberToLcd` / `python`
