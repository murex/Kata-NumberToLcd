# Getting Started with Number to LCD kata in Go
...
> ***Reminder***: the commands below should be run from [Kata-NumberToLcd/go]() directory
...
> ***Reminder***: the command below should be run from [Kata-NumberToLcd/go]() directory
...
`File` > `Open` > `Kata-NumberToLcd` > `go`

Run all the tests in [lcd_test.go](./src/lcd/lcd_test.go).
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-NumberToLcd` / `go`
