# Getting Started with Number to LCD kata in C++
...
- Create a build directory: ***Kata-NumberToLcd/cpp/build***. All build-related files are generated under this directory.
...
- Generate the solution file ***Kata-NumberToLcd.sln*** for **Visual Studio 2017** on Windows,
  or the project file ***Kata-NumberToLcd.xcodeproj*** for **Xcode** on macOS.
...
> ***Reminder***: the commands below should be run from [Kata-NumberToLcd/cpp]() directory
...
> ***Reminder***: the command below should be run from [Kata-NumberToLcd/cpp]() directory
...
1. Make sure the 'Kata-NumberToLcd-test' project is set as the Start-up Project.
...
`File` > `Open` > `Kata-NumberToLcd` > `cpp`

CLion should automatically build the CMake file.

Run the test file [LcdTest.cpp](./test/LcdTest.cpp)
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-NumberToLcd` / `cpp`
...
Open Xcode, choose `Open a project or file`, navigate to `Kata-NumberToLcd` / `cpp` / `build` / `Kata-NumberToLcd.xcodeproj`
then click `Open`.

Once the project is opened in Xcode, select `Product` / `Scheme` / `Kata-NumberToLcd-test` to set it as the current active scheme.
